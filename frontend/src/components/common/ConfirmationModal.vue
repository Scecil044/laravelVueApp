    ...
    <div v-if="open" class="fixed inset-0 z-50 flex items-center justify-center bg-black/40 min-h-screen">
      <div :class="[danger ? 'border-red-200 dark:border-red-500' : 'border-brand', 'bg-white dark:bg-dark-bg rounded-2xl shadow-xl p-5 w-full max-w-md text-sm border-2 flex flex-col items-center']">
        <div class="flex flex-col items-center mb-4">
          <div :class="[danger ? 'bg-red-100 text-red-500 border-red-200 dark:bg-red-900 dark:text-red-400 dark:border-red-500' : 'bg-brand/10 text-brand border-brand', 'rounded-full border-4 w-12 h-12 flex items-center justify-center mb-2']">
            <i :class="[icon || (danger ? 'pi pi-exclamation-circle' : 'pi pi-question-circle'), 'text-3xl']"></i>
          </div>
          <h3 class="text-lg font-extrabold mb-1 text-gray-900 dark:text-white text-center">{{ title }}</h3>
        </div>
        <p class="text-gray-600 dark:text-gray-300 mb-4 text-center text-sm">{{ message }}</p>
